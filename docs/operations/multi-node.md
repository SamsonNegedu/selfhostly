...
> **Read this first.** New secondary machines now connect **out** to the primary and need no open port: run
> `selfhostlyctl join-token` on the primary and the single `join` command it prints on the new machine
> (see [deploy.md#add-a-secondary-node](deploy.md#add-a-secondary-node) and [rfcs/node-link.md](../rfcs/node-link.md)). The setup
> steps below describe the older **direct** mode (the primary calls each secondary over your network, and every
> machine needs a reachable address and port). It is still supported and is what existing nodes use, but it is no
> longer the recommended way to add a machine. Where this page says a secondary "must be reachable", that applies
> to direct mode only.
...
- [Cloudflare Zero Trust Setup](cloudflare-zero-trust.md)
- [GitHub Whitelist Authentication](../security/github-allowlist.md)
