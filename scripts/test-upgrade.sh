...
sqlite3 "$WORK/data/selfhostly.db" "INSERT INTO apps (id,name,description,compose_content,status,node_id) VALUES ('a1','kan','','x','running',(SELECT id FROM nodes LIMIT 1))"
...
[[ "$(sqlite3 data/selfhostly.db 'SELECT COUNT(*) FROM apps WHERE name = "kan"')" == 1 ]] && pass "app record survived" || fail "app record survived"
[[ "$(sqlite3 data/selfhostly.db 'SELECT MAX(version) FROM schema_migrations')" -ge 1 ]] && pass "database migrated" || fail "database migrated"
...
[[ "$(sqlite3 data/selfhostly.db 'SELECT COUNT(*) FROM apps')" == 1 ]] && pass "data intact after rollback" || fail "data intact after rollback"
docker tag selfhostly-upg-backend:new "$LIVE_BACKEND"
...
[[ "$(sqlite3 data/selfhostly.db "SELECT COUNT(*) FROM nodes WHERE api_key LIKE 'enc:v1:%'")" -ge 1 ]] && pass "node keys are now encrypted" || fail "node keys are now encrypted"
"${UPGRADE[@]}" --set SECURITY_MODE=enforce > up7.log 2>&1 && pass "switch to enforce" || { fail "switch to enforce"; tail -8 up7.log; }
[[ "$(sqlite3 data/selfhostly.db "SELECT COUNT(*) FROM nodes WHERE api_key LIKE 'enc:v1:%'")" -ge 1 ]] && pass "changing the mode did not touch the encryption state" || fail "mode change altered encryption"
"${UPGRADE[@]}" --set ENCRYPT_SECRETS_AT_REST=false > up8.log 2>&1 && pass "turn encryption off" || { fail "turn encryption off"; tail -8 up8.log; }
[[ "$(sqlite3 data/selfhostly.db "SELECT COUNT(*) FROM nodes WHERE api_key LIKE 'enc:v1:%'")" == 0 ]] && pass "node keys converted back to plaintext (rollback-safe)" || fail "conversion back to plaintext"
healthy primary && pass "healthy" || fail "healthy"

echo; echo "T5d identity is adopted from the database, not from a compose default"
DB_ID="$(sqlite3 data/selfhostly.db 'SELECT id FROM nodes WHERE is_primary = 1')"
rm -f data/node-id
"${UPGRADE[@]}" --set NODE_ID= > up9.log 2>&1 && pass "restart with NODE_ID unset" || { fail "restart with NODE_ID unset"; tail -8 up9.log; }
[[ "$(sqlite3 data/selfhostly.db 'SELECT id FROM nodes WHERE is_primary = 1')" == "$DB_ID" ]] && pass "database identity unchanged" || fail "database identity unchanged"
