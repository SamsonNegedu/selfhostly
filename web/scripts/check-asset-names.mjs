...
// those rules match anywhere in the path, so /assets/api-abc123.js would be sent to the gateway and answer 404.
...
const RISKY = new RegExp(`/(${GATEWAY_PREFIXES.join('|')})`)
...
if (risky.length > 0) {
    ...
    console.error(`No path segment may start with ${GATEWAY_PREFIXES.join(', ')}. Adjust the file names in vite.config.ts.`)
    process.exit(1)
}
