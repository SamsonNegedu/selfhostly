...
function FleetAppRow({ fleetApp, metrics, actions }: FleetAppRowProps) {
    ...
    return (
        <Card className={cn('relative flex flex-col gap-2 p-3', isFailed && 'border-status-err/50')} data-app={app.name}>
            <div className="flex items-center gap-3">
                <AppTile name={app.name} size="md" />
                <div className="min-w-0 flex-1">
                    <Link to={appHref(app)} className="block truncate text-[15px] font-semibold after:absolute after:inset-0 after:content-['']">
                        {app.name}
                    </Link>
                ...
                <div className="relative z-10 flex items-center">
                    {canStart && (
                        <Button variant="ghost" size="icon" aria-label={`${isFailed ? 'Retry start' : 'Start'} ${app.name}`} onClick={() => actions.start(app)} disabled={busy}>
                            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}
                        </Button>
                    )}
                    {inProgress && <Loader2 aria-label="Working" className="mx-3 h-4 w-4 animate-spin text-muted-foreground" />}
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-44">
                            ...
                            <DropdownMenuItem onSelect={() => actions.requestDelete(app)} disabled={busy} className="text-destructive focus:text-destructive">
