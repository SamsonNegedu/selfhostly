...
function FleetToolbar({ counts, filter, onFilterChange, query, onQueryChange, groupBy, onGroupByChange, showGroupBy }: FleetToolbarProps) {
    const chips = CHIPS.filter((chip) => chip.alwaysShow || counts[chip.value] > 0 || chip.value === filter)

    return (
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
            <div role="group" aria-label="Filter by status" className="flex gap-2 overflow-x-auto scrollbar-hide md:flex-wrap md:overflow-visible xl:flex-1">
                {chips.map((chip) => {
                    ...
                    return (
                        ...
                        >
                            {chip.label}
                            <span className={cn('text-xs', active ? 'opacity-80' : 'text-muted-foreground')}>{counts[chip.value]}</span>
                        </button>
                    )
                })}
            </div>
