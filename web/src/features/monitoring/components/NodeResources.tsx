...
function Resource({ label, kind, percent, detail, history }: { label: string; kind: ResourceKind; percent: number; detail: string; history: number[] }) {
        ...
        <div className="flex flex-col gap-2">
            <div className="flex items-baseline justify-between gap-2">
                ...
                <span className="flex items-center gap-2">
                    <StatusPill kind={tone} size="sm">{TONE_WORD[tone]}</StatusPill>
            ...
            {history.length < 2 && <p className="h-14 text-[12.5px] text-muted-foreground">Collecting readings. The trend appears in a few seconds.</p>}
            {history.length >= 2 && <AreaChart data={history} tone={tone} height={56} label={`${label} since you opened this page`} />}
        </div>
    )
}
    ...
    return (
        <Card className="flex flex-col gap-4 p-4" data-node-stats={stats.node_name}>
            ...
            <div className="grid gap-5 sm:grid-cols-3">
                <Resource label="CPU" kind="cpu" percent={stats.cpu.usage_percent} detail={`${stats.cpu.cores} ${stats.cpu.cores === 1 ? 'core' : 'cores'}`} history={history?.cpu ?? []} />
                <Resource label="Memory" kind="memory" percent={stats.memory.usage_percent} detail={`${formatBytes(stats.memory.used_bytes)} of ${formatBytes(stats.memory.total_bytes)}`} history={history?.memory ?? []} />
                <Resource label="Disk" kind="disk" percent={stats.disk.usage_percent} detail={`${formatBytes(stats.disk.used_bytes)} of ${formatBytes(stats.disk.total_bytes)}`} history={history?.disk ?? []} />
            </div>
        </Card>
    )
}
