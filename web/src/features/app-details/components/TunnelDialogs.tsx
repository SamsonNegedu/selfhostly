...
export function QuickTunnelDialog({ open, onOpenChange, appId, nodeId, composeContent, recreate = false }: DialogProps & { recreate?: boolean }) {
    ...
    const submit = (event: React.FormEvent) => {
        ...
        if (!Number.isInteger(portNumber) || portNumber < 1 || portNumber > MAX_PORT) return setError(`Enter a port from 1 to ${MAX_PORT}.`)
        setError(null)
        create.mutate(
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>A temporary public address on trycloudflare.com. It can change when the app restarts, so it suits trying things out, not production.</DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="flex flex-col gap-4">
                    <Field label="Service" hint="The service name in your compose file that listens for web traffic.">
                        <Input value={service} onChange={(event) => setService(event.target.value)} placeholder="web" autoComplete="off" />
                    </Field>
                    ...
                    {error && <p role="alert" className="text-sm text-status-err-fg">{error}</p>}
...
export function CustomDomainDialog({ open, onOpenChange, appId, nodeId, composeContent, switching = false }: DialogProps & { switching?: boolean }) {
    const { toast } = useToast()
    ...
    const change = (index: number, patch: Partial<IngressRule>) => setRules(rules.map((rule, i) => (i === index ? { ...rule, ...patch } : rule)))
    ...
    const submit = (event: React.FormEvent) => {
        ...
        if (!filled.some((rule) => rule.hostname && rule.hostname.trim() !== '')) return setError('At least one route needs a hostname, your own domain, such as app.example.com.')
        setError(null)
        mutation.mutate(
            { appId, nodeId, ingressRules: filled.map((rule) => ({ hostname: rule.hostname || undefined, service: rule.service.trim(), path: rule.path || undefined })) },
            {
                onSuccess: () => {
                    onOpenChange(false)
                    toast.info(switching ? 'Switching to your domain' : 'Creating your tunnel', 'This runs in the background. The new address appears here when it is ready.')
                },
                onError: (failure) => setError(describeError(failure)),
            }
        )
                        ...
                        {rules.map((rule, index) => (
                            <fieldset key={index} className="flex flex-col gap-3 border-t border-border pt-4 first:border-t-0 first:pt-0">
                                ...
                                <Field label={rules.length > 1 ? `Hostname (route ${index + 1})` : 'Hostname'}>
                                    <Input value={rule.hostname ?? ''} onChange={(event) => change(index, { hostname: event.target.value.trim() || null })} placeholder="app.example.com" inputMode="url" />
                                </Field>
                                <Field label="Service address" hint="Where the app answers, for example http://web:80.">
                                    <Input value={rule.service} onChange={(event) => change(index, { service: event.target.value.trim() })} placeholder="http://web:80" inputMode="url" className="font-mono" />
                                </Field>
                                <Field label="Path (optional)">
                                    <Input value={rule.path ?? ''} onChange={(event) => change(index, { path: event.target.value.trim() || null })} placeholder="/" />
                                </Field>
                                {rules.length > 1 && (
