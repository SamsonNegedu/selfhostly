...
// A fixed moment, so the sample jobs below do not read the clock while rendering.
...
const LONG_LINE =
    '      - LONG_VARIABLE=https://example.com/a/very/long/path/that/keeps/going/and/going/until/it/is/wider/than/any/phone/screen/could/hold'
const LOG_LINES: TerminalLine[] = [
    ...
    ...Array.from({ length: 10 }, (_, i) => ({
        time: `17:08:${10 + i}`,
        level: 'info' as const,
        text: `GET /status.php 200 ${i + 2}ms`,
    })),
]
...
function BoundaryDemo() {
    ...
    return (
        <div data-row="error-boundary" className="flex flex-col gap-3">
            <Button variant="outline" className="self-start" onClick={() => setBroken(true)}>
                Break this part
            </Button>
    ...
    return (
        <div className="space-y-8">
            ...
            <Section id="radio" title="Radio cards">
                ...
                <p className="mt-3 text-xs text-muted-foreground" data-testid="radio-state">
                    Selected: {exposure}
                </p>
            </Section>
            ...
            <Section id="segmented" title="Segmented control">
                <div className="flex items-center gap-4">
                    ...
                    />
                    <span className="text-sm text-muted-foreground" data-testid="segmented-state">
                        View: {view}
                    </span>
                </div>
            </Section>
            ...
            <Section id="select" title="Select">
                ...
                <p className="mt-3 text-xs text-muted-foreground" data-testid="select-state">
                    Node: {node}
                </p>
            </Section>
            ...
            <Section id="status" title="Status pills and dots">
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3" data-row="pills-sm">
                        {STATUS_KINDS.map((kind) => (
                            <StatusPill key={kind} kind={kind} size="sm">
                                {kind}
                            </StatusPill>
                        ))}
                    </div>
                    <div className="flex flex-wrap items-center gap-3" data-row="dots">
                        {STATUS_KINDS.map((kind) => (
                            <span key={kind} className="flex items-center gap-2 text-sm">
                                <StatusDot kind={kind} />
                                {kind}
                            </span>
                        ))}
                    ...
                    <div className="space-y-2" data-row="mapped">
                        <div className="flex flex-wrap gap-3">
                            {APP_STATUSES.map((s) => (
                                <StatusPill key={s} kind={appStatusMeta(s).kind}>
                                    {appStatusMeta(s).label}
                                </StatusPill>
                            ))}
                        </div>
                        <div className="flex flex-wrap gap-3">
                            {NODE_STATUSES.map((s) => (
                                <StatusPill key={s} kind={nodeStatusMeta(s).kind}>
                                    {nodeStatusMeta(s).label}
                                </StatusPill>
                            ))}
                            {JOB_STATUSES.map((s) => (
                                <StatusPill key={s} kind={jobStatusMeta(s).kind}>
                                ...
                                <TableRow key={name}>
                                    <TableCell>
                                        <span className="flex items-center gap-3">
                                            <AppTile name={name} size="sm" />
                                            <span className="font-medium">{name}</span>
                                        </span>
                                    </TableCell>
                                    <TableCell>
                                        <StatusPill kind={index === 2 ? 'idle' : 'ok'}>
                                            {index === 2 ? 'Stopped' : 'Running'}
                                        </StatusPill>
                                    </TableCell>
                                    <TableCell className="text-muted-foreground">{NODE_NAMES[index]}</TableCell>
                                    <TableCell>
                                        <Sparkline data={CPU_SERIES} width={60} height={20} />
                                    </TableCell>
            ...
            <Section id="code" title="Code, diff and terminal">
                <div className="space-y-5">
                    ...
                    <div data-row="diff">
                        <DiffBlock before={COMPOSE_BEFORE} after={COMPOSE_AFTER} aria-label="Changes to compose.yml" />
                    </div>
                    <div data-row="long">
                        <CodeBlock code={LONG_LINE} aria-label="Long line" />
                    </div>
                ...
                <div className="space-y-5">
                    ...
                    <div data-row="error">
                        <ErrorState
                            ...
                            error={
                                new Error(
                                    '{"error":"Failed to get app logs","details":"container operation failed: get app logs"}',
                                )
                            }
                            onRetry={() => setRetries((n) => n + 1)}
                        />
                        <p className="mt-2 text-xs text-muted-foreground" data-testid="retry-count">
                            Retries: {retries}
                        </p>
                    </div>
            ...
            <Section id="fleet-states" title="Fleet: loading and first run">
                <div className="space-y-8">
                    <div data-row="fleet-loading">
                        <FleetLoading />
                    </div>
                    <div data-row="fleet-empty">
                        <FleetEmpty />
                    </div>
                </div>
            </Section>
                                ...
                                job={
                                    {
                                        ...
                                        error_message:
                                            status === 'failed' ? 'port is already allocated: 0.0.0.0:8082' : undefined,
                                        started_at: new Date(GALLERY_NOW - 90_000).toISOString(),
                                        completed_at:
                                            status === 'completed' || status === 'failed'
                                                ? new Date(GALLERY_NOW - 30_000).toISOString()
                                                : undefined,
                                        created_at: new Date(GALLERY_NOW - 100_000).toISOString(),
                                    } as never
                                }
            ...
            <Section id="overlays" title="Dialogs, sheets, tooltip, menu, toasts">
                <div className="space-y-5">
                    <div className="flex flex-wrap items-center gap-3" data-row="dialogs">
                        <Button variant="outline" onClick={() => setConfirmOpen('restart')}>
                            Restart dialog
                        </Button>
                        <Button variant="danger" onClick={() => setConfirmOpen('delete')}>
                            <Trash2 className="h-4 w-4" />
                            Delete dialog
                        </Button>
                        <Sheet>
                        ...
                        <TooltipProvider>
                            <TooltipTrigger asChild>
                                <Button variant="outline" aria-label="Restart app">
                                    <RotateCw className="h-4 w-4" />
                                </Button>
                            </TooltipTrigger>
                    ...
                    <div className="flex flex-wrap items-center gap-3" data-row="toasts">
                        <Button
                            variant="outline"
                            onClick={() => toast.success('nextcloud restarted', 'Back online in 9 seconds')}
                        >
                            Success toast
                        </Button>
                        <Button
                            variant="outline"
                            onClick={() =>
                                toast.error('Could not start vaultwarden', 'Port 8082 is already in use', {
                                    label: 'Fix port',
                                    onClick: () => log('toast:fix-port'),
                                })
                            }
                        >
                            Error toast
                        </Button>
                        <Button variant="outline" onClick={() => toast.warning('Memory is at 84%')}>
                            Warning toast
                        </Button>
                        <Button variant="outline" onClick={() => toast.info('Syncing tunnel routes')}>
                            Info toast
                        </Button>
                    </div>
                    <div
