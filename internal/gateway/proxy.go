...
func (p *Proxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	// If no X-Forwarded-Host, try to extract from Referer (handles Vite proxy scenario)
	if forwardedHost == "" {
	...
	// For auth callback with no Referer (GitHub redirect), check if we stored the origin host in a cookie
