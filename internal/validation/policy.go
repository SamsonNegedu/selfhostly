...
type PolicyResult struct {
	Hard []string
	Soft []string
	// Allowable lists the Soft violations that adding one host folder to ALLOWED_VOLUME_PATHS resolves.
	// Their messages are also in Soft; this carries the folder so callers never have to read it back
	// out of message text.
	Allowable []AllowableViolation
}

// AllowableViolation is a Soft violation fixed by allowing HostPath
type AllowableViolation struct {
	Message  string
	HostPath string
}
...
// softAllowable records a Soft violation that allowing hostPath would resolve
func (r *PolicyResult) softAllowable(hostPath, format string, a ...interface{}) {
	msg := fmt.Sprintf(format, a...)
	r.Soft = append(r.Soft, msg)
	r.Allowable = append(r.Allowable, AllowableViolation{Message: msg, HostPath: hostPath})
}
...
func classifyHostPath(p string) string {
	for _, d := range pathsDeniedWithSubtree {
		if p == d {
			return fmt.Sprintf("%s must never be mounted into an app", p)
		}
		if isWithin(p, d) {
...
// splitVolumeSpec splits a short-syntax volume ("source:target[:mode]") on its colons, leaving alone the
// colons inside a variable's default, as in "${DATA_DIR:-./data}:/app/data".
...
func (c *policyCtx) checkBindSource(what, src string) {
	...
	c.res.softAllowable(abs, "%s %q is outside the app directory and ALLOWED_VOLUME_PATHS", what, src)
}
